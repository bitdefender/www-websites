...
(() => {
  ...
  // _src/scripts/sw-src.js
  var VERSION = "bd-tools-v1";
  ...
  var OFFLINE_FALLBACK = "/pwa-offline.html";
  self.skipWaiting();
  clientsClaim();
  var navigationStrategy = new NetworkFirst({
  ...
  self.addEventListener("activate", (event) => {
    event.waitUntil(
      caches.keys().then((keys) => Promise.all(
        keys.filter((key) => ![PAGE_CACHE, ASSET_CACHE, IMAGE_CACHE].includes(key)).map((key) => caches.delete(key))
      ))
    );
  });
  registerRoute(
