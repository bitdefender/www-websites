...
export default async function decorate(block, options) {
  ...
  const desktopImageEL = desktopImage.querySelector('img');

  await new Promise((resolve) => {
    if (desktopImageEL && !desktopImageEL.complete) {
      desktopImageEL.setAttribute('loading', 'eager');
      desktopImageEL.addEventListener('load', resolve);
      desktopImageEL.addEventListener('error', resolve);
    } else {
      resolve();
    }
  });
